---
title: "Em Coffee House"
city: "Elk Grove"
address: "7440 Laguna Blvd, Suite 111, Elk Grove, CA 95758"
phone: "(916) 897-9967"
website: "https://www.emcoffeehouse.com/"
category: "Coffee & Tea"
date_visited: "2026-08-06"
tags: ["matcha", "cafe", "elk-grove", "coffee", "tea", "matcha-flight"]
status: "draft"
---

# Em Coffee House

**Where:** 7440 Laguna Blvd, Suite 111, Elk Grove, CA 95758
**Phone:** (916) 897-9967 · **Web:** [emcoffeehouse.com](https://www.emcoffeehouse.com/)
**Category:** Coffee & Tea café

Em Coffee House in Elk Grove is a bright, modern café that's a great
stop for matcha lovers. The standout is their **matcha flight** (the
"Em Experience" flight tasting set, $23 dine-in) — a smart, low-commitment
way to taste four different styles of matcha side by side (four pours at
$5.75 each, $23 for the set).

## The Matcha Flight

The flight arrives as four small footed tasting glasses lined up on a
long wooden paddle board, each one individually labeled. It's a beautiful
presentation, and each pour is nicely done and distinct from the others:

1. **Ube Matcha** — layered purple ube and green matcha over milk; the
   most eye-catching of the set.
2. **Matcha Latte** — a clean, classic iced matcha layered over milk.
3. **Matcha Einspänner** — matcha topped with a thick, velvety cream cap
   (Vienna/einspänner style).
4. **Matcha Coconut Blast** — crowned with whipped cream and toasted
   coconut flakes for a dessert-like finish.

Every pour was good and unique. The flight format made it a fun way to
explore the range — from a straightforward matcha latte to the creamy
einspänner and the more playful ube and coconut versions — without
committing to a full drink of each. A great way to try different types of
matcha in one sitting.

## Notes

- Bright, modern, airy space with lots of greenery — though the plants
  are all faux, not live.
- The four-glass wooden paddle presentation is a real highlight.
- Solid pick for anyone wanting to branch out beyond a standard matcha
  latte, or to share and compare with a friend.
